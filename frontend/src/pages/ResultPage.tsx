        ...
        <div>
          ...
          <ol className="flex flex-col gap-1 list-none">
            {summary.recommendations.map((r, i) => (
              <li key={i} className="flex items-start gap-2 text-sm text-slate-400">
                <span className="text-amber-500 shrink-0">{i + 1}.</span>{r}
              </li>
            ))}
          </ol>
        </div>
...
export function ResultPage() {
  ...
  useEffect(() => {
    // All three fetches run in parallel — no sequential GUI blocking
    Promise.all([
  ...
  const overallAvg     = Object.values(result.average_score).reduce((a, b) => a + b, 0) / 5
  const grade          = getGrade(overallAvg)
  const pastInterviews = history.filter(h => h.interview_id !== interviewId).slice(0, 3)
  ...
  return (
    <div className="min-h-screen flex flex-col items-center px-4 py-10">
      <div className="w-full max-w-lg flex flex-col gap-6">
        ...
        {/* Competency Map — radar chart, aggregated across all past interviews */}
        {competency && <CompetencyMap profile={competency} />}
        ...
        {pastInterviews.length > 0 && (
          <div className="flex flex-col gap-3">
            <div className="flex items-center justify-between">
              ...
              <Link to="/profile" className="text-xs text-indigo-400 hover:text-indigo-300 transition-colors">
                Все интервью →
              </Link>
            </div>
            {pastInterviews.map(h => (
              <Link
                key={h.interview_id}
              ...
              >
                <div className="flex items-center gap-3">
                  <span className={`text-xs px-2 py-0.5 rounded border font-medium ${levelBadge(h.level)}`}>
                    {h.level}
                  </span>
                  <span className="text-sm text-slate-400">{h.stack}</span>
